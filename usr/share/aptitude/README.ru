...
Как я могу получить aptitude?
  ...
  Сборка aptitude из исходного кода
   ...
   выполните команду tar zxf aptitude-0.8.7.tar.gz для того, чтобы
   распаковать исходный код. Когда исходный код будет распакован, введите cd
   aptitude-0.8.7 && ./configure && make для того, чтобы скомпилировать
...
Терминальный пользовательский интерфейс aptitude
  ...
  Меню команд
   ...
   | Действия → Отменить        | removals, upgrades, holds, marking as     |
   | незаконченные действия     | automatically installed...). This is      |
...
+-------------------------------------------------------------------------------------------------------+
   ...
   ?not(шаблон), !шаблон
           ...
           [Примечание] Примечание
                        ...
                        чтобы выбрать пакеты, чье описание содержит «extra!»,
   ...
   ?or(шаблон1, шаблон2), шаблон1 | шаблон2
           ...
           [Примечание] Примечание
                        Чтобы использовать символ «|» в регулярном выражении,
                        он должен быть «экранирован», иначе aptitude создаст
                        из него термин ИЛИ: «~|».
   ...
   ?action(действие), ~aдействие
           ...
           [Примечание] Примечание
                        Заметьте, что этот запрос проверяет лишь то,
                        действительно ли действие будет произведено над
                        пакетом, а не то, может ли вообще оно быть
                        произведено. Таким образом, например,
                        ?action(upgrade) соответствует только тем пакетам,
                        которые вы уже решили обновить, а не тем пакетам,
                        которые могут быть обновлены в будущем (для
   ...
   ?any-version(шаблон)
           ...
           [Примечание] Примечание
                        ...
                        packages but versions of the packages which are
                        matched. For «aptitude search» and other uses it does
                        not make much difference, but «aptitude versions»
                        will only show the versions that match, not all
                        versions of the package for which any version
   ...
   ?upgradable, ~U
           ...
           [Примечание] Примечание
                        ...
                        available for a package, not whether an upgrade could
                        be performed (with actions such as safe-upgrade or
                        full-upgrade).

                        For instance, there can be upgrades available for
                        several packages, but maybe the upgraded versions of
                        a subset conflict with each other, so they cannot be
                        co-installed or not all of them upgraded at the same
                        time.
...
Настройка aptitude
 ...
 %[ширина][?]код[#]
   ...
   %%            Литерал %       1            Нет          просто вставляет
                                                           ...
                                                           строке, группы,
                                                           соответствующие
   %#число       Замена          Переменная   Нет          поиску,
                 параметра                                 используются в
                                                           качестве
                                                           параметров при
                                                           отображении
                                                           результата поиска.
   ...
   %D            Размер пакета   8            Нет          файла, содержащего
                                                           пакет.
                                                           Outputs the string
   %E            Architecture    10           Нет          of the
                                                           architecture, for
                                                           example «amd64».
                                                           Outputs the source
                                                           package, for
   %e            Source          30           Нет          example «aptitude»
                                                           for
                                                           «aptitude-doc-en».
                                                           Имя компьютера, на
   ...
   %M            автоматической  1            Нет          автоматически,
                 установки                                 выводит «A»; в
                                                           ...
                                                           Выводит версию
                                                           aptitude,
   %n            Версия          Длина        Нет          работающую в
                 программы       «0.8.7».                  настоящий момент,
                                                           в настоящее время
                                                           «0.8.7».
                                                           Выводит имя
   ...
   %o            загрузки        17           Нет          пакетных файлов,
                                                           ...
                                                           Outputs a string
                                                           of the origin of
   %O            Origin          30           Нет          the package, for
                                                           example
                                                           «Debian:unstable
                                                           [amd64]».
                                                           Выводит имя
   ...
   %r            Число обратных  2            Нет          число
                 зависимостей                              установленных
                                                           ...
                                                           Выводит
                                                           сокращенное
                 Приоритет в                               описание
   %R            сокращенном     3            Нет          приоритета пакета:
                 виде                                      например,
                                                           «Important»
                                                           заменяется на
                                                           «Imp».
   ...
   %t            Архив           10           Да           Архив, в котором
                                                           ...
                                                           пакет отмечен
                                                           тегом, в противном
                 Tagged (and                               случае не выводит
   %T            user-tags)      30           Нет          ничего.^[19]
 ...
 firstchar[(mode)]
           ...
           режим может быть одним из следующих:
                ...
                binary
                        ...
                        Пример 2.7. Grouping policy firstchar or
                        firstchar(binary)
 ...
 pattern(шаблон [=> заголовок] [{ правило }] [, ...])
           ...
           Пример 2.10. Использование pattern с некоторыми пакетами,
           ...
           Пример 2.11. Использование политики группировки pattern с
           подполитиками
 ...
 versions
   ...
   installsizechange
           ...
           выполнения установки), сравнивая текущую версию (если она
           установлена) и версию-кандидата, которая будет установлена,
  ...
  Настройка привязки клавиш
   ...
   |------------------------+-----------------+-----------------------------|
   |                        |                 | Сворачивает выбранное       |
   | CollapseTree           | left            | дерево в иерархическом      |
   |                        |                 | списке.                     |
   |------------------------+-----------------+-----------------------------|
   ...
   |                        |                 | иерархическом списке.       |
   |------------------------+-----------------+-----------------------------|
   | ExpandTree             | right           | Открывает выбранное дерево  |
 ...
 Aptitude {
   ...
   По-умолчанию: false
   Описание: If this option is true, aptitude will clean up obsolete files
   (see Действия → Очистить устаревшие файлы) every time you update the
   ...
   package cache directory (see Действия → Очистить кэш пакетов) after
   ...
   По-умолчанию: false
   Описание: If this option is enabled, aptitude will use the user's locale
   for messages and dates in the log file; otherwise it is written in the
   ...
   По-умолчанию: -300
   Описание: How much weight the problem resolver should give to not remove
   ...
   По-умолчанию: -10
   Описание: How much to reward or penalize prospective solutions based on
...
Действия командной строки
   ...
   install
           ...
           [Примечание] Примечание
                        ...
                        the «install» command will modify aptitude's stored
                        information about what actions to perform. Therefore,
                        if you issue (e.g.) the command «aptitude install foo
                        bar» on packages previously uninstalled, and then the
                        installation fails once aptitude has started
                        downloading and installing packages, you will need to
                        run «aptitude remove foo bar» to go back to the
                        previous state (and possibly undo installations or
   ...
   search
           ...
           [Примечание] Примечание
                        ...
                        аргумента после search и, поэтому, производится поиск
                        по двум шаблонам: «~N» и «edit». Как это описано в
                        руководстве по поисковым шаблонам, один шаблон,
                        составленный из двух подшаблонов, разделённых
                        пробелом (такой как «~N edit»), соответствует
                        какому-то пакету только в том случае, если оба эти
                        шаблона соответствуют пакету. Таким образом, команда
                        «aptitude search '~N edit'» покажет только «новые»
                        пакеты, чьи имена содержат «edit».
 ...
 ihA raptor-utils                    - утилиты для анализа и преобразования RDF
   ...
   show
           ...
           Отображает подробную информацию об одном или нескольких пакетах.
           Если имя пакета содержит символ тильды («~») или знак вопроса
           («?»), то оно будет считаться поисковым шаблоном, и будут
           отображены все подпадающие под него пакеты (см. раздел «Поисковые
           запросы» в руководстве aptitude).
...
Опции
 ...
 libdbix-fulltextsearch-perl 0.73-10
   ...
   -F формат, --display-format формат
           ...
           search и versions. Например, передача «%p %v %V» для format
           приведёт к отображению имени пакета, за которым следует
           установленная в данный момент версия и версия-кандидат (см. раздел
           «Настройка того, как отображаются пакеты» в руководстве aptitude).
...
Описание
   ...
   [Примечание] Примечание
                ...
                exceptional cases, it is not intended for end-users under
